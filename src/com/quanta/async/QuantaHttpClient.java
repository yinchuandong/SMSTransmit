...
public class QuantaHttpClient {
    ...
    private void saveCookies(){
    	...
    	try {
    		...
    		for (Cookie cookie : cookies) {
    			...
    			obj.put("expiryDate", cookie.getExpiryDate() == null ? "" : cookie.getExpiryDate());
    		...
    		SharedPreferences sp = QuantaAppUtil.getSharedPreferences(this.context);
    ...
    private void addCookies(){
    	if (cookieStore != null) {
			...
			SharedPreferences sp = QuantaAppUtil.getSharedPreferences(this.context);
			String cookieStr = sp.getString(QuantaConfig.PERSISTENT_COOKIE, null);
			if (cookieStr == null) {
				return;
			}
			...
			try {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					if (expiryDate != null && !expiryDate.equals("")) {
						SimpleDateFormat format = new SimpleDateFormat("EEE MMM dd HH:mm:ss 'GMT' yyyy", Locale.US);
    ...
    public void debugMemory(String tag) {
		try {
			...
			Log.w(this.getClass().getSimpleName()+":"+method,
					tag + ":" + QuantaAppUtil.getUsedMemory());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
